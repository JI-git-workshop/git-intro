...
* 在这个项目的根目录你还会发现一个叫`python.pdf`的文档，这是我很久以前写的一个python入门教程，里面有一点关于命令行的介绍（有兴趣可以看�?
...
找到适合自己系统的版�?32位或64�?，运行安装程序即可。安装设置一路next下去即可，只要保证环境变量那一页的设置是第二个就行了�?
...
* 在你的terminal运行`xcode-select --install`安装`Xcode Command Line Tools`。这是Apple官方的一整套开发工具（还包含了gcc，g++）等工具�?*推荐使用该方法下�?*。下载时间可能较长，建议睡前插着电源让它下载�?
...
由于其他网站Orgnization人数受限，为了演示和实践方便还是请大家注册一个Github账号�?国内上这个网站比较慢，但是等一会还是能出来的，推荐chrome或者firefox浏览器，别用IE就行。有梯子的同学可以开起来�?

前往[Github](https://github.com)创建一个账�?Sign Up): 
...
然后，登陆到Github，去右上角找到你的账号settings里的SSH and GPG keys，选择增加一个新的ssh-key，title可以随便输入你喜欢的名字，底下的打输入框中，复制你刚刚在命令行中打印出来的公钥内�?�?ssh-rsa"开始一直到你的邮箱结束)。然后点击底部按钮即可添加完成�?
...
Git GUI简单来说就�?*一�?*可以免去你敲命令行的痛苦的桌面软件。它与Git的关系仅仅是锦上添花，没有GUI软件你照样可以用命令行里的Git完成所有操作。当然GUI软件会增加一些实用的工具�?
...
### 关于Sourcetree初始�?添加ssh-key

#### 如果你前面生成ssh-key成功�?

请前往`工具->选项->一般`里，将默认的ssh客户端换成`OpenSSH`，选择你刚刚生成的密钥对中�?*私钥（id_rsa�?*，并**重启sourcetree**。一般默认的路径就是对的，不用改�?
